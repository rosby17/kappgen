...
export default function App() {
  ...
  const [profileTab, setProfileTab] = useState('security'); // 'security' | 'appearance'
  ...
  return (
    <div className="font-body-md antialiased overflow-hidden flex h-screen bg-background text-on-background">
      ...
      <nav className="hidden md:flex flex-col bg-surface-container text-primary font-label-bold text-label-bold fixed left-0 top-0 h-screen w-[240px] z-40 border-r border-outline-variant py-xl">
        ...
        {/* Bottom Sidebar Profile Card (Cleaned: no Supabase DB widget, no logout button here) */}
      ...
      <main className="flex-1 flex flex-col md:ml-[240px] h-screen overflow-hidden bg-background">
        ...
        <div className="flex-1 overflow-y-auto p-gutter md:p-xl">
          <div className="max-w-[1440px] mx-auto space-y-8">
            ...
            {/* REFINED AUTHENTICATION MODAL (Google Login + Email/Password + Show/Hide Password Eye + Forgot Password) */}
            {showAuthModal && (
            ...
            {/* REFINED PROFILE & SETTINGS MODAL (Password Change, Theme Switch, 2FA, Logout) */}
            {showProfileModal && currentUser && (
              <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-6">
                <div className="bg-level-1 rounded-xl p-8 max-w-[560px] w-full max-h-[90vh] overflow-y-auto border border-surface-container-highest shadow-2xl">
                  <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 bg-surface-container-lowest p-1.5 rounded-xl mb-6 border border-surface-container-highest">
                    <button 
                      ...
                      className={`py-2 rounded-lg text-xs font-label-bold flex items-center justify-center gap-2 transition-all ${profileTab === 'security' ? 'bg-primary-container text-on-primary-container font-bold shadow' : 'text-on-surface-variant'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`py-2 rounded-lg text-xs font-label-bold flex items-center justify-center gap-2 transition-all ${profileTab === 'appearance' ? 'bg-primary-container text-on-primary-container font-bold shadow' : 'text-on-surface-variant'}`}
                    >
                  ...
                  {/* Logout Button (Moved strictly inside profile settings) */}
