  ...
  const handleCreateChannel = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchChannelVideos(created.id);
      }
    } catch (e) {
      alert("Erreur lors de la création de la chaîne: " + e.message);
    } finally {
      setLoading(false);
    }
  };
          ...
          <button 
            onClick={() => {
              if (channels.length > 0) {
                if (!activeChannel) setActiveChannel(channels[0]);
      ...
      <main className="flex-1 flex flex-col md:ml-[240px] h-screen overflow-hidden bg-background">
        ...
        <div className="hidden md:flex justify-between items-center px-xl py-6 border-b border-outline-variant">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => { setWizardStep(1); setView('wizard'); }}
