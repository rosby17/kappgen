...
# ── Bring-your-own-key for the AI text-generation providers ────────────────
# Every provider KappGen can call for script/text generation (see
# src/pipeline/ai_providers.py) — a creator who connects their own key here
# has it used in that provider's place wherever it sits in the fallback
# chain (src/pipeline/ai_text.py), free of charge against their KappGen
# credits. Izivoice (voice) already has its own dedicated connect/disconnect
# route in channels.py with its own UI — listed here too (read-only status)
# so Settings shows one single "toutes mes clés API" place, without moving
# its actual storage/logic.
_TEXT_PROVIDER_IDS = ("anthropic", "kie", "deepseek", "fal", "openai")


@router.get("/me/provider-keys")
def list_provider_keys(current_user: User = Depends(get_current_user)):
    from src.pipeline.ai_providers import PROVIDERS
    entries = current_user.external_ai_keys or {}
    by_id = {p["id"]: p for p in PROVIDERS}
    result = [
        {
            "provider_id": pid,
            "label": by_id[pid]["label"],
            "connected": bool((entries.get(pid) or {}).get("encrypted")),
            "key_prefix": (entries.get(pid) or {}).get("prefix"),
            "enabled": (entries.get(pid) or {}).get("enabled", True),
            "managed_elsewhere": False,
        }
        for pid in _TEXT_PROVIDER_IDS
    ]
    # Izivoice shown alongside for one unified list, pointing the frontend at
    # its own existing endpoints (POST/DELETE /channels/izivoice/connect)
    # rather than duplicating its storage.
    result.append({
        "provider_id": "izivoice",
        "label": "Izivoice (voix off)",
        "connected": bool(current_user.izivoice_api_key_encrypted),
        "key_prefix": current_user.izivoice_key_prefix,
        "enabled": True,
...
@router.post("/me/provider-keys/{provider_id}")
def set_provider_key(provider_id: str, payload: ProviderKeyPayload, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if provider_id not in _TEXT_PROVIDER_IDS:
    ...
    from src.utils.credentials import encrypt_credential, key_prefix
    entries = dict(current_user.external_ai_keys or {})
    entries[provider_id] = {"encrypted": encrypt_credential(raw_key), "prefix": key_prefix(raw_key), "enabled": True}
    current_user.external_ai_keys = entries
    db.commit()
    return {"provider_id": provider_id, "connected": True, "key_prefix": entries[provider_id]["prefix"]}


@router.delete("/me/provider-keys/{provider_id}")
def remove_provider_key(provider_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    entries = dict(current_user.external_ai_keys or {})
...
class ProviderKeyTogglePayload(BaseModel):
    enabled: bool


@router.put("/me/provider-keys/{provider_id}/toggle")
def toggle_provider_key(provider_id: str, payload: ProviderKeyTogglePayload, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    entries = dict(current_user.external_ai_keys or {})
    if provider_id not in entries:
        raise HTTPException(status_code=404, detail="Aucune clé connectée pour ce fournisseur.")
    entries[provider_id] = {**entries[provider_id], "enabled": payload.enabled}
    current_user.external_ai_keys = entries
    db.commit()
