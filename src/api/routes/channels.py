...
def update_channel(channel_id: str, payload: ChannelUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if payload.image_style is not None:
        ...
        # max_unique_images doubles as the generation budget on a channel with
        # AI generation enabled (see resolve_generated_image_budget), so it
        # maps to real money and is clamped here rather than trusted from the
        # request — the UI's own max is not an enforcement point. Left as-is
        # when unset ("auto"), which lets the pipeline pick from the video's
        # own length. Which ENGINE serves those images is
        ...
        for field in ("max_unique_images", "premium_image_count"):
            raw_count = image_style.get(field)
            if raw_count is None:
                continue
            try:
                count = int(raw_count)
            except (TypeError, ValueError):
                image_style[field] = None
                continue
            image_style[field] = min(count, premium_image_count_ceiling()) if count > 0 else None
...
def delete_channel(channel_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # cascade="all, delete-orphan"); every other table carrying a channel_id
    # has to be cleared by hand or the delete fails on the foreign key. That
    # list used to be maintained inline here and fell behind as new tables
