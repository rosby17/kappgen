...
def fetch_own_channel_info(access_token: str) -> Optional[dict]:
    ...
    replace KappGen's placeholder channel identity with the creator's real
...
def upload_video(
    ...
    contains_synthetic_media self-certifies YouTube's "altered or synthetic
    content" disclosure — every KappGen video is AI-generated (script,
    voice, and/or visuals), so this defaults to True per YouTube's policy.
    """
    ...
    if tags:
        snippet["snippet"]["tags"] = [str(t)[:100] for t in tags][:500]
...
def set_video_thumbnail(access_token: str, video_id: str, thumbnail_path: Path) -> None:
    """Sets the custom thumbnail generated by KappGen for an uploaded video."""
...
def publish_video_for_channel(
    ...
    if thumbnail_path and thumbnail_path.exists():
        try:
            ...
            # The video is already live at this point. Never turn a successful
            # upload into a retry that could publish a duplicate.
            logger.warning(f"Video {video_id} published, but custom thumbnail upload failed: {exc}")
    return video_id
